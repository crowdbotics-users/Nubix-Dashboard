/*jshint esversion: 6 */
const margin2 = {top:0, right:20, bottom:30, left:60};
...
for (var i=0; i<data2.length; i++) {
  data2[i].ts = new Date(data2[i].ts);
}

let parentWidth2 = $('#wrapper2').parent().width();
let parentHeight2 = $('#wrapper2').parent().height();
let width2 = parentWidth2 - margin2.left - margin2.right;
let height2 = parentHeight2 - margin2.top - margin2.bottom;

let svg2 = d3.select('#wrapper2');
let g2 = svg2.append('g2')
  .attr('transform', 'translate('+margin2.left+','+margin2.top+')');

// set ranges
let x2 = d3.scaleTime().range([0, width2]);
let y2 = d3.scaleLinear().range([height2, 0]);

// scale the range of data2
